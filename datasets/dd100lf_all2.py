...
class DD100lfAll2(Dataset):
        ...
        def _process_files(agent_files, np_music):       
            ...
            for dtype_folder in dtypes:
                ...
                if not self.full_length:
                    for i in range(0, seq_len - self.max_gt_length, 8):
                        np_dance_sub_seq_l = ldance[i: i + self.max_gt_length]
                        np_dance_sub_seq_f = fdance[i: i + self.max_gt_length]
                        np_music_sub_seq = np_music[i: i + self.max_gt_length]

                        if len(np_dance_sub_seq_l) != self.max_gt_length or len(np_dance_sub_seq_f) != self.max_gt_length or len(np_music_sub_seq) != self.max_gt_length:
                            continue
    ...
    def __getitem__(self, index):
        ...
        music = torch.from_numpy(music[:-1]).float()
